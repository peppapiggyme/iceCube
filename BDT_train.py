...
def Train(X, y, error, errorx, tree_args, boosting_args, tag):
    ...
    pickle.dump(clf, open(os.path.join(
        MODEL_PATH, f"BDT_clf.{tag}.sklearn"), "wb"))
...
if __name__ == "__main__":
    ...
    columns = ["kappa", "zenith", "error",
               "sumq", "qz", "dt_50", "dt_85", "ez"]
    ...
    # input
    X = np.concatenate(
        [reco, xe, ev1, ev2, ev3, vavg, evvv, vvvv, distq], axis=1)
    ...
    threads.append(
        threading.Thread(target=Train, args=(
            X, idx, error, errorx, tree_args, boosting_args, "Baseline.0414"))
    )
    ...
    threads.append(
        threading.Thread(target=Train, args=(
            X, idx, error, errorx, tree_args, boosting_args, "Deep.0414"))
    )
    ...
    threads.append(
        threading.Thread(target=Train, args=(
            X, idx, error, errorx, tree_args, boosting_args, "BaseMore.0414"))
    )
    ...
    threads.append(
        threading.Thread(target=Train, args=(
            X, idx, error, errorx, tree_args, boosting_args, "BaseMoreMore.0414"))
    )
